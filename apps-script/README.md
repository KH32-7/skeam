...
- 웹 게임 클라우드 저장은 `saves` 탭에 있습니다 (처음 저장할 때 자동으로 생김). 한 줄이 세이브 한 버전이고, 게임마다 최근 3개와 1시간 전, 하루 전 것 하나씩을 남깁니다. 데이터는 압축된 글자라 열어 봐도 읽을 수 없고, 손으로 고치면 안 됩니다. 한 사람의 세이브를 지우려면 그 사람(`lower` 칸) 줄을 지웁니다.
